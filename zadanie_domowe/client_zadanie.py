...
class PeopleClient:
    ...
    def delete_by_id(self, user_id):
        response = requests.delete(self.base_url+str(user_id))
        response.raise_for_status()
        if response.status_code != 200:
            raise PeopleClientError(response.json()['Uknown id'])
        return response.json()
...
if __name__ == '__main__':
    ...
    print(adresses)
    usuniete = client.delete_by_name('Jaime')
    #print(usuniete)
    #usuniete_by_id = client.delete_by_id('57')

    with open('osoby.json', 'rt') as input_json_file:
        dodane = client.add_from_file(input_json_file)
